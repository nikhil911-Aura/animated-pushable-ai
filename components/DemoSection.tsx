...
const feed = [
  ...
  { done: true,  text: "Lead follow-up email scheduled for 9am",    time: "14s ago",  color: "#f97316" },
  { done: true,  text: "Onboarding document sent to candidate",     time: "1m ago",   color: "#fb923c" },
  { done: true,  text: "Weekly P&L report compiled",                time: "3m ago",   color: "#f97316" },
  { done: false, text: "Summarising team standup notes…",           time: "running",  color: "#fb923c" },
];
  ...
  return (
    <section id="demo" className="py-28 relative bg-white">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 right-0 w-96 h-96 bg-orange-400/6 rounded-full blur-[110px] -translate-y-1/2" />
      </div>
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-8 relative">
        ...
        <div ref={gridRef} className="grid lg:grid-cols-2 gap-6 items-start" style={{ willChange: "transform" }}>
              ...
              <div className="p-3 space-y-1.5">
                {feed.map((f, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-orange-50 transition-colors"
                  >
                    {f.done ? (
                      <CheckCircle2 className="w-4 h-4 shrink-0" style={{ color: f.color }} />
                    ) : (
                      <div className="w-4 h-4 rounded-full border-2 border-orange-400 border-t-transparent shrink-0 animate-spin" />
                    )}
                    <span className={`text-[12px] flex-1 ${f.done ? "text-gray-600" : "text-orange-600"}`}>{f.text}</span>
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-900 overflow-hidden">
              ...
              <div className="p-3 space-y-1">
                {logs.map((l, i) => (
                  ...
                  >
                    ...
                    <span className={
                      ...
                      l.ok === false ? "text-orange-400 animate-pulse" :
                                       "text-gray-500"
                    }>{l.msg}</span>
                  </motion.div>
                ))}
                <div className="flex items-baseline gap-3 text-[11px] mono">
                  ...
                  <span className="text-gray-600">▌<span className="animate-blink">_</span></span>
                </div>
              </div>
            </div>
          ...
          >
                ...
                <div className="absolute inset-0 p-5 flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-md bg-orange-500/30 flex items-center justify-center">
                        <Zap className="w-3 h-3 text-orange-400" />
                      </div>
                      <span className="text-[11px] text-gray-400 mono">Pushable AI — Live Operations</span>
                    </div>
                  ...
                  <div className="space-y-2.5 flex-1">
                    {[
                      { label: "Invoice Processing", w: "78%", c: "#f97316" },
                      { label: "Lead Follow-ups",    w: "92%", c: "#fb923c" },
                      { label: "HR Onboarding",      w: "61%", c: "#f97316" },
                      { label: "Task Scheduling",    w: "85%", c: "#fb923c" },
                      { label: "Report Generation",  w: "44%", c: "#f97316" },
                    ].map((row, i) => (
            ...
            {/* Bottom glow */}
            <div className="absolute -bottom-4 inset-x-8 h-8 bg-orange-500/10 blur-xl rounded-full pointer-events-none" />
          </motion.div>
        </div>
      </div>
    </section>
